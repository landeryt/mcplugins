    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (sender instanceof Player p) {
            if (!cooldown.containsKey(p.getUniqueId())
                ...
                System.out.println(ChatColor.AQUA + "Welcome back!");
            } else {
                p.sendMessage(ChatColor.YELLOW + "You have to wait " + ChatColor.RED
                        + (double)(10000 - System.currentTimeMillis()
                        - cooldown.get(p.getUniqueId()))/100 + ChatColor.YELLOW
                        + " seconds before you can do this command.");
            }
