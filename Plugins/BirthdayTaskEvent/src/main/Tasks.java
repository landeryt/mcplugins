...
public class Tasks implements Listener {
    ...
    // Medium
    @EventHandler
    public void onPillagerKill(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player p && e.getEntity() instanceof Pillager pillager) {
            if (check("pillagerKill", p) && pillager.getHealth() > 0) return;
            ItemStack item = p.getInventory().getItemInMainHand();
            if (item.getType().equals(Material.CROSSBOW)) {
                if (tokenCheck(p, 2)) {
                    output(p, "pillagerKill");
                    Bukkit.getServer().broadcastMessage(p.getDisplayName() + ChatColor.YELLOW
                            + " has completed the MEDIUM task: Kill a pillager with crossy.");
                }
            }
        }
    }
    ...
    // Easy
    @EventHandler
    public void onEndermanKill(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player p && e.getEntity() instanceof Enderman enderman) {
            if (check("endermanKill", p)) return;
            if (enderman.getHealth() <= 0) {
                if (tokenCheck(p, 1)) {
    ...
    // Hard
    @EventHandler
    public void onSnowmanKill(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player p && e.getEntity() instanceof Snowman snowman) {
            if (check("snowmanKill", p)) return;
            Biome biome = p.getLocation().getBlock().getBiome();
            if (snowman.getHealth() <= 0 && biome.equals(Biome.DESERT)) {
                if (tokenCheck(p, 3)) {
    ...
    public void onEnderchestBreak(BlockBreakEvent e) {
        ...
        Material block = e.getBlock().getType();
        ItemStack item = p.getInventory().getItemInMainHand();
        if (block.equals(Material.ENDER_CHEST) && !item.containsEnchantment(Enchantment.SILK_TOUCH)) {
            if (tokenCheck(p, 2)) {
